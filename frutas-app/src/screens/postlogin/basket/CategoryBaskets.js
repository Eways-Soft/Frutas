...
export default function CategoryScreen({ navigation }) {
  ...
  const [catId, setcatId] = React.useState(route.params.catId);
  ...
  async function getBasketsForCurrentCat() {
    ...
    fetch(`${BASE_URL}getcategorybaskets`, {
      method: 'POST',
      headers: {
        Accept: 'application/json',
        'Content-Type': 'application/json'
                        ...
                        <View style={styles.productlistitem_addview}>
                          {item.discount_price > 0 ?
                            <View>
                              ...
                              <Text style={[styles.productlistitem_price, styles.font_semibold]}><FIcon name="rupee" size={13} color="#000" />{item.actual_price}</Text>
                            ...
                            :
                            <Text style={[styles.productlistitem_price, styles.font_semibold]}><FIcon name="rupee" size={13} color="#000" />{item.basket_sale_price}</Text>
                          }
